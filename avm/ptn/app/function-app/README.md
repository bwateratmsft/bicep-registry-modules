...
Application settings (`name`/`value` pairs) to merge into the Function App configuration. All values must be strings. Reserved keys managed by this module are silently dropped to keep the Function App in a working state — see `reservedAppSettingKeys` in `main.bicep` for the current list.
...
| Parameter | Type | Description |
| :-- | :-- | :-- |
| [`>Any_other_property<`](#parameter-appsettingskeyvaluepairs>any_other_property<) | string | The string value of the application setting. |

### Parameter: `appSettingsKeyValuePairs.>Any_other_property<`

The string value of the application setting.
...
### Parameter: `flexConsumptionInstanceMemoryMB`

(Flex Consumption only) Memory allocated to each instance of the Function App in MB. Allowed values are 512, 2048, and 4096.
...
### Parameter: `flexConsumptionMaximumInstanceCount`

(Flex Consumption only) Maximum number of instances the Function App can scale out to. Allowed range is 40-1000.
...
The runtime stack of the Function App, e.g. `dotnet-isolated`, `node`, `python`, `java`, `powershell`. Note: `dotnet` (in-process .NET) is **not** supported on Flex Consumption (`FC1`); use `dotnet-isolated` instead.
...
- Required: No
- Type: string
- Default: `[take(toLower(format('{0}sa{1}', replace(parameters('functionAppName'), '-', ''), uniqueString(resourceGroup().id, parameters('functionAppName')))), 24)]`

### Parameter: `tags`

Resource tags to apply to all created resources. The runtime Storage Account is always tagged with `resource-usage: azure-functions`.
