...
@description('Optional. Number of workers for the App Service Plan. Also sets the maximum elastic worker count for Elastic Premium so its ceiling is not below the requested capacity.')
@minValue(1)
...
// or unsupported state, as documented in the parameter description.
...
var userAssignedIdentityClientId = createUserAssignedIdentity
...
@description('The resource ID of the User-Assigned Managed Identity used by the Function App for runtime storage access and Application Insights ingestion (created by this module or supplied via `userAssignedIdentityResourceId`).')
...
output userAssignedIdentityClientId string = userAssignedIdentityClientId
